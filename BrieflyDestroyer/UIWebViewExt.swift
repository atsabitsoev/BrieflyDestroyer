...
extension WKWebView {
    ...
    @discardableResult
    func callJSMethod(name: String,
                      agruments: StringConvertable...,
                      handler: @escaping (_ data: String?, _ error: Error?) -> ()) -> String?{
        ...
        self.evaluateJavaScript(js) { (data, error) in
            let str = data ?? error?.localizedDescription
            newString = str as? String
            handler(newString, error)
        }
        return newString
    ...
    @objc func webView(_ webView: AnyObject!, didCreateJavaScriptContext context: JSContext!, forFrame frame: AnyObject!)
    {
        let notifyDidCreateJavaScriptContext = {() -> Void in
            for exportData in __jsExportDatas
            {
                ...
                webView.evaluateJavaScript("var \(checksum) = '\(checksum)'") { (data, error) in
                    print(error ?? data ?? "чегоооооо")
                }
...
class JSExportData{
    ...
    init(webView: WKWebView, export: JSExport, key: String = "Native") {
